...
m %>%
  ...
  # # 
  # # summarise(order,time,position_check)%>%
  # arrange(time)%>%
  # 
  # mutate(position_diff = c(0,diff(position_check)))%>%
  # 
  ...
  # 
  # mutate(position_diff = ifelse(time_diff ==0, position_diff, 0)) %>%
  # 
  # mutate(location_model1 = ifelse(-4 <position_diff & position_diff<0 & time_diff ==0, , position_check))%>%
  # 
  # 
...
new%>%
  expand(new,time = seq(0,16,0.01))%>%
  
  mutate(position_null = ifelse(( time <= walking_time_min+order*4/walking_speed),walking_speed*time-4*order,0))%>%
  
  
  mutate(position_2 = ifelse(((walking_time_min+order*4/walking_speed)< time & time<=(sit_duration_new+ walking_time_min+order*4/walking_speed)),walking_distance,0))%>%
  
  mutate(position_check = ifelse(position_null == 0, position_2, position_null))%>%
  
  mutate(position_check2 = ifelse(time>(sit_duration_new+ walking_time_min+order*4/walking_speed), NA, position_check))%>% 
